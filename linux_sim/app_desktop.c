...
static float s_scroll_tgt = 0;   /* 目标+插值=丝滑 */
static void on_scroll_view(fx_widget_t *w, void *ud) {
    ...
    s_scroll_tgt -= fx_wheel_take(w);   /* 统一: -= dy */          /* 目标像素 */
if (s_scroll_tgt < 0) s_scroll_tgt = 0;
if (s_scroll_tgt > maxs) s_scroll_tgt = maxs;
s_scroll_off += (int)((s_scroll_tgt - s_scroll_off) * 0.25f);   /* 逐帧插值 */
if (s_scroll_tgt - s_scroll_off > -1 && s_scroll_tgt - s_scroll_off < 1) s_scroll_off = (int)s_scroll_tgt;
...
void build_desktop_pages(void)
{
    ...
    /* ---- 页6 画板 ---- */
    fx_canvas_new(pixel("6,32", "330,236"), name("paint_cv"), page(6), anim(1), color(FX_WHITE), name("paint_cv"), call(on_paint));
...
fx_parent(fx_find("move_cv"));   /* 移动件挂画布下, 单遍绘制 */
